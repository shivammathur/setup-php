...
handle_dependency_extensions() {
  ...
  if [[ -n "$suffix" ]]; then
    ...
    patch_abstract_file 
    for dependency_extension in "${dependency_extensions[@]}"; do
        safe_brew install "${brew_opts[@]}" "$ext_tap/$dependency_extension@$version"  && copy_brew_extensions "$dependency_extension"
    done
  fi
}
...
add_brew_extension() {
  ...
  if check_extension "$extension"; then
    ...
    (safe_brew install "${brew_opts[@]}" "$ext_tap/$formula@$version"  && copy_brew_extensions "$formula") || pecl_install "$extension" 
