...
add_php() {
  ...
  update_dependencies
  sudo rm -rf "$core_tap"/Formula/php@* "$core_tap"/Formula/php.rb
  if brew list php@"$version" >/dev/null 2>&1 && [ "$action" != "upgrade" ]; then
...
core_tap="$tap_dir"/homebrew/homebrew-core
